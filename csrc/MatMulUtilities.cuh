 ...
 ***************************************************************************/
// Extended from https://github.com/AlibabaResearch/flash-llm/blob/main/csrc/MatMulUtilities.cuh
                                                                ...
                                                                bool        Pred = true)
{
    ...
    //
    int       warp_id            = threadIdx.x / 32;
    int       TotalNumOfCopyUnit = NumOfRowsToCopy / COPY_UNIT_FP16_ROWS_16;
    const int MaxIteration =
        (TotalNumOfCopyUnit - 1) / (TilingConfig::BLOCK_ROW_WARPS * TilingConfig::BLOCK_COL_WARPS) + 1;
#pragma unroll
    for (int i = 0; i < MaxIteration; i++) {
        int  COPY_UNIT_I        = (i * (TilingConfig::BLOCK_ROW_WARPS * TilingConfig::BLOCK_COL_WARPS) + warp_id);
        bool AsyncCopyPredictor = COPY_UNIT_I < TotalNumOfCopyUnit && Pred;
        const half* GlobalPTR_Unit        = GlobalPTR + COPY_UNIT_I * COPY_UNIT_FP16_ROWS_16 * GlobalStride;  
        half* __restrict__ SharedPTR_Unit = SharedPTR + COPY_UNIT_I * COPY_UNIT_FP16_ROWS_16 * TILE_K_HALF;
        cp_async<16>(SharedPTR_Unit + store_column1 * HALF_PER_128B + row1 * TILE_K_HALF,
                     GlobalPTR_Unit + col * HALF_PER_128B + row1 * GlobalStride,
                     AsyncCopyPredictor);
                                                                ...
                                                                bool        Pred = true)
{
    ...
    //
    int       warp_id            = threadIdx.x / 32;
    int       TotalNumOfCopyUnit = NumOfRowsToCopy / 4;
    const int MaxIteration =
        (TotalNumOfCopyUnit - 1) / (TilingConfig::BLOCK_ROW_WARPS * TilingConfig::BLOCK_COL_WARPS) + 1;
#pragma unroll
    for (int i = 0; i < MaxIteration; i++) {
        int  COPY_UNIT_I        = (i * (TilingConfig::BLOCK_ROW_WARPS * TilingConfig::BLOCK_COL_WARPS) + warp_id);
        bool AsyncCopyPredictor = COPY_UNIT_I < TotalNumOfCopyUnit && Pred;
        const uint32_t* GlobalPTR_Unit        = GlobalPTR + COPY_UNIT_I * 4 * GlobalStride;
        uint32_t* __restrict__ SharedPTR_Unit = SharedPTR + COPY_UNIT_I * 4 * TILE_K_HALF;
        cp_async<16>(SharedPTR_Unit + col * UINT32_PER_128B + row * TILE_K_HALF,
                     GlobalPTR_Unit + col * UINT32_PER_128B + row * GlobalStride,
                     AsyncCopyPredictor);
    }
}
                                                                ...
                                                                bool        Pred = true)
{
    ...
    //
    int       warp_id            = threadIdx.x / 32;
    int       TotalNumOfCopyUnit = NumOfRowsToCopy / 2;
    const int MaxIteration =
        (TotalNumOfCopyUnit - 1) / (TilingConfig::BLOCK_ROW_WARPS * TilingConfig::BLOCK_COL_WARPS) + 1;
#pragma unroll
    for (int i = 0; i < MaxIteration; i++) {
        int  COPY_UNIT_I        = (i * (TilingConfig::BLOCK_ROW_WARPS * TilingConfig::BLOCK_COL_WARPS) + warp_id); 
        bool AsyncCopyPredictor = COPY_UNIT_I < TotalNumOfCopyUnit && Pred;
        const uint32_t* GlobalPTR_Unit        = GlobalPTR + COPY_UNIT_I * 2 * GlobalStride;
        uint32_t* __restrict__ SharedPTR_Unit = SharedPTR + COPY_UNIT_I * 2 * TILE_K_HALF;
        cp_async_8<8>(SharedPTR_Unit + col * UINT32_PER_64B + row * TILE_K_HALF,
                     GlobalPTR_Unit + col * UINT32_PER_64B + row * GlobalStride,
                     AsyncCopyPredictor);
    }
}
...
#pragma unroll
    for (int i = 0; i < MaxIteration; i++) {
        int  COPY_UNIT_I        = (i * (TilingConfig::BLOCK_ROW_WARPS * TilingConfig::BLOCK_COL_WARPS) + warp_id);
        bool AsyncCopyPredictor = COPY_UNIT_I < TotalNumOfCopyUnit && Pred;
        const uint32_t* GlobalPTR_Unit        = GlobalPTR + COPY_UNIT_I * GlobalStride;
        uint32_t* __restrict__ SharedPTR_Unit = SharedPTR + COPY_UNIT_I * TILE_K_HALF;
        cp_async_8<4>(SharedPTR_Unit + col * UINT32_PER_64B + row * TILE_K_HALF,
                     GlobalPTR_Unit + col * UINT32_PER_64B + row * GlobalStride,
                     AsyncCopyPredictor);
    }
}
                                                               ...
                                                               bool        Pred = true)
{
    ...
    //
    int       warp_id            = threadIdx.x / 32;
    int       TotalNumOfCopyUnit = NumOfRowsToCopy / 8;
    const int MaxIteration =
        (TotalNumOfCopyUnit - 1) / (TilingConfig::BLOCK_ROW_WARPS * TilingConfig::BLOCK_COL_WARPS) + 1;
#pragma unroll
    for (int i = 0; i < MaxIteration; i++) {
        int  COPY_UNIT_I        = (i * (TilingConfig::BLOCK_ROW_WARPS * TilingConfig::BLOCK_COL_WARPS) + warp_id);
        bool AsyncCopyPredictor = COPY_UNIT_I < TotalNumOfCopyUnit && Pred;
        const uint64_t* GlobalPTR_Unit        = GlobalPTR + COPY_UNIT_I * 8 * GlobalStride;
        uint64_t* __restrict__ SharedPTR_Unit = SharedPTR + COPY_UNIT_I * 8 * TILE_BITMAP_K;
        cp_async<16>(SharedPTR_Unit + col * UINT64_PER_128B + row * TILE_BITMAP_K,
                     GlobalPTR_Unit + col * UINT64_PER_128B + row * GlobalStride,
                     AsyncCopyPredictor);
    }
}
// New features: Copy size is X * 8 Uint64, X can be any multiple to 8 // for Bitmap
                                                               ...
                                                               bool        Pred = true)
{
    ...
    int       warp_id            = threadIdx.x / 32;
    int       TotalNumOfCopyUnit = NumOfRowsToCopy;  //   
    bool AsyncCopyPredictor = row < TotalNumOfCopyUnit && Pred && warp_id == 0;  
                                                               ...
                                                               bool        Pred = true)
{
    ...
    const uint64_t* GlobalPTR_Unit        = GlobalPTR;  
    uint64_t* __restrict__ SharedPTR_Unit = SharedPTR; 
    cp_async<16>(SharedPTR_Unit + col * UINT64_PER_128B + row * TilingConfig::TILE_BITMAP_K_V1, 
                     GlobalPTR_Unit + col * UINT64_PER_128B + row * GlobalStride,   
                     AsyncCopyPredictor);
}
template<int NumOfRowsToCopy, typename TilingConfig>  // NumOfRowsToCopy must be multiple to COPY_UNIT_FP16_ROWS
                                                               ...
                                                               bool        Pred = true)
{
    ...
    const uint64_t* GlobalPTR_Unit        = GlobalPTR;  
    uint64_t* __restrict__ SharedPTR_Unit = SharedPTR; 
    cp_async<16>(SharedPTR_Unit + col * UINT64_PER_128B + row * TilingConfig::TILE_BITMAP_K_V3, 
                     GlobalPTR_Unit + col * UINT64_PER_128B + row * GlobalStride,   
                     AsyncCopyPredictor);
}
